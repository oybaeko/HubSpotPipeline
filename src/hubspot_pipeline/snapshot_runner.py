...
from .process_snapshot import process_snapshot, process_score_history_for_snapshot
...
def register_snapshot(snapshot_id, triggered_by="manual", status="completed", notes=None):
    ...
    if errors:
        logging.info(f"❌ Failed to register snapshot: {errors}")
    else:
...
def run_snapshot_and_process(filters_company=None, filters_deal=None, triggered_by="manual", limit=1000):
    ...
    try:
        companies = fetch_companies(filters=filters_company, limit=limit)
        ...
        logging.info(f"✅ Snapshot run {snapshot_id} completed.")
    except Exception as e:
        register_snapshot(snapshot_id, triggered_by=triggered_by, status="error", notes=str(e))
        logging.info(f"❌ Snapshot run {snapshot_id} failed: {e}")
